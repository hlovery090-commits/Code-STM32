%% train_ann_1layer_v7base.m
% Single-hidden-layer ANN replacement of Iq PI controller
% (converted from train_ann_full_replace_v7.m, 2-layer [10 8] -> 1-layer [20])
% Input : I_D_MEAS, I_Q_MEAS, I_Q_REF, ENCODER_SPEED
% Output: V_Q
% Architecture: 4 -> [20 tansig] -> 1 purelin
clear; clc;

%% ---- STEP 1: Load log data (matches MotorPilot export format) ----
LOG_FILE = 'LOG0-100%__4.csv';   % <-- update to your file
...
%% ---- STEP 3: Train/Val/Test split & network (SINGLE hidden layer) ----
net = fitnet(20, 'trainlm');   % <-- was fitnet([10 8], ...) in v7; now one layer of 20
net.divideParam.trainRatio = 0.70;
net.divideParam.valRatio   = 0.15;
net.divideParam.testRatio  = 0.15;

net.layers{1}.transferFcn = 'tansig';
net.layers{2}.transferFcn = 'purelin';   % output layer is now layer 2 (was layer 3 in v7)
...
plotregression(testY, testPred, 'ANN-FF 4-20-1 (Motor Pilot log)');
...
% Graph 4: Residual (error) distribution -- shows where the ANN
...
% Graph 5: Error vs Speed -- shows whether error grows with speed
figure('Name', 'Error vs Speed');
scatter(testX(4,:), abs(residuals), 8, 'filled', 'MarkerFaceAlpha', 0.3);
xlabel('ENCODER\_SPEED');
ylabel('|Prediction error| (V)');
title('Absolute error across speed range');
grid on;

%% ---- STEP 6: Extract normalization + weights (SINGLE layer) ----
xmin = net.inputs{1}.processSettings{1}.xmin;
xmax = net.inputs{1}.processSettings{1}.xmax;
xgain = 2 ./ (xmax - xmin);
xoffset = xmin;

ymin = net.outputs{2}.processSettings{1}.xmin;   % output is layer 2 now (was layer 3)
ymax = net.outputs{2}.processSettings{1}.xmax;
ygain = 2 ./ (ymax - ymin);
yoffset = ymin;

IW1 = net.IW{1};    % 20 x 4
b1  = net.b{1};     % 20 x 1
LW2 = net.LW{2,1};  % 1 x 20
b2  = net.b{2};     % 1 x 1

%% ---- STEP 7: Export to C header (single-hidden-layer format) ----
% Matches ann_1layer.h / ann_1layer.c (ANN1L_* naming), NOT
% ann_foc_full.h (that one expects two hidden layers, ANNFULL_* naming).
fid = fopen('ann_1layer_weights.h', 'w');
fprintf(fid, '/* Auto-generated by train_ann_1layer_v7base.m (converted from v7, single hidden layer) */\n');
fprintf(fid, '/* Architecture: 4 -> [20 tansig] -> 1 purelin | Test R = %.5f */\n', R(1,2));
fprintf(fid, '#ifndef ANN_1LAYER_WEIGHTS_H\n#define ANN_1LAYER_WEIGHTS_H\n\n');
fprintf(fid, '#define ANN1L_N_INPUTS   4\n');
fprintf(fid, '#define ANN1L_N_HIDDEN   20\n');
fprintf(fid, '#define ANN1L_N_OUTPUTS  1\n\n');

writeArr = @(name, M) fprintf(fid, 'static const float %s[%d] = {%s};\n\n', ...
    name, numel(M), strjoin(compose('%.8ff', M(:)'), ', '));

writeArr('ANN1L_X_GAIN',   xgain);
writeArr('ANN1L_X_OFFSET', xoffset);
fprintf(fid, 'static const float ANN1L_Y_GAIN = %.8ff;\n', ygain);
fprintf(fid, 'static const float ANN1L_Y_OFFSET = %.8ff;\n\n', yoffset);

writeArr('ANN1L_IW1', IW1');   % row-major flatten: [hidden][input]
writeArr('ANN1L_B1',  b1);
writeArr('ANN1L_LW2', LW2);
fprintf(fid, 'static const float ANN1L_B2 = %.8ff;\n\n', b2);

fprintf(fid, '#endif // ANN_1LAYER_WEIGHTS_H\n');
fclose(fid);

fprintf('Exported ann_1layer_weights.h (4-20-1)\n');
